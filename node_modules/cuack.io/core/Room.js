...
/**
 * Handle the namespace authorization for this room
 * @see https://github.com/LearnBoost/socket.io/wiki/Authorizing
 * 
 * @example function(handshakeData, callback){
 *	callback(null, true);
 * }
 */
Room.prototype.handleAuthorization = function(handshakeData, callback){
	callback(null, true);
};
/**
 * @throweable
...
Room.prototype.create = function(){
	...
	this.room.authorization(function(handshakeData, callback){
		me.handleAuthorization(handshakeData, callback);
	});
	
	this.room.on('connection', function(socket){
		me.emit('newClient', socket);
console.log('NEW CLIENT on ROOM', me.name, socket.id);
		me.clients[socket.id] = socket;

		socket.on('disconnect', function(){
			me.emit('clientOut', socket);
			if('function' === typeof(me.onDisconnect)){
		...
		if('function' === typeof(me.onConnection)){
			me.onConnection.apply(me, arguments);
		}
	});
}
...
console.log('EVENT LISTENER: room:' + me.name + ': ', methodName, arguments);
		 ...
		 */
		me.beforeEventListener(methodName, args);
		ret = me[methodName].apply(
			me, 
			args
		);
		me.afterEventListener(methodName, args);
		return ret;
	};
};
...
//console.log('RPC: room:' + me.name + ': ', methodName, arguments);
		 ...
		 * {...} rest of parameters sended by Client-Side
		 */
		me.beforeRPC(methodName, args);
		ret = me[methodName].apply(
			me, 
			args
		);
		me.afterRPC(methodName, args);
		return ret;
	};
};

Room.prototype.beforeRPC = function(methodName, args){}
Room.prototype.afterRPC = function(methodName, args){}
Room.prototype.beforeEventListener = function(methodName, args){}
Room.prototype.afterEventListener = function(methodName, args){}
